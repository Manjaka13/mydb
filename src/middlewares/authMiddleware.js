...
const Hauth = require("../services/hauth");
...
const isMaster = (req, res, next) => {
	if (res.locals.user.level === 0) next();
	else res.json(failure("You need to be MASTER to access this route"));
};

// Protects routes from people that aren't MASTER or ADMIN
const isAdmin = (req, res, next) => {
	if (res.locals.user.level < 2) next();
	else res.json(failure("You need to be admin to access this route"));
};

// Protects routes from unconfirmed accounts
const isConfirmed = (req, res, next) => {
	if (res.locals.user.status === 1) next();
